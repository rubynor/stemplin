class TimeRegsController < ApplicationController
  ...
  def index
    ...
    # Per-day totals for the week strip, derived from the authorized scope above.
    @minutes_by_day = @time_regs_week.group_by(&:date_worked).transform_values { |regs| regs.sum(&:current_minutes) }
